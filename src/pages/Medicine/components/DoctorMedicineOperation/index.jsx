...
const DoctorMedicineOperation = () => {
  ...
  const { run: cancelProxy, loading: cancelProxyLoading } = useRequest(
    apis.medicine.cancelProxy,
    {
      ...
      onError() {
        alert(<Text className="text-red">取消授权失败</Text>);
      }
    }
  );

  const { run: cancelGift, loading: cancelGiftLoading } = useRequest(
    apis.medicine.cancelGift,
    {
      manual: true,
      onSuccess() {
        alert("取消转赠成功");
      ...
      onError() {
        alert(<Text className="text-red">取消转赠失败</Text>);
      }
    }
  );
    ...
    <MedicineBgView>
      ...
      <View className="grid grid-cols-2 w-full gap-6 mt-[48px]">
        <Button
          ...
          type="primary"
          block
          disabled={doctor?.proxy === true || doctor?.selfuse === false}
        >
          自用领药申请
        </Button>
        <Button
      ...
      {doctor.proxy === false && doctor.selfuse === null && (
